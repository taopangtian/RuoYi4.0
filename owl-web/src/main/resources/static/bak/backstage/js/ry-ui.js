...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            importFile:function(formId,importUrl,postfixs){
            	if(!postfixs)return;
            	...
            	layer.open({
            		...
            		btn1: function(index, layero){
            			...
            			var isfix=false;
	                	$.each(postfixs, function(index, fix) {
	                		if (file == '') return false;
	                		if($.common.endWith(file, fix))
	                		{
	                			isfix=true;
	                			return false;
	                		}
	                	...
	                	if (!isfix){
            				$.modal.msgWarning("请选择后缀为 ["+postfixs.join('、')+"]的文件。");
            				return false;
            			}
            ...
            importExcel: function(formId) {
            	$.table.importFile(formId,$.table._option.importUrl,['.xls','.xlsx']);
            },
            // 刷新表格
            refresh: function() {
        ...
        operate: {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.Backstage_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
