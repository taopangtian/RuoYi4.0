...
一直想做一款后台管理系统，看了很多优秀的开源项目但是发现没有合适的。于是利用空闲休息时间开始自己写了一套后台系统。如此有了Jack.Lin。她可以用于所有的Web应用程序，如网站管理后台，网站会员中心，CMS，CRM，OA。所有前端后台代码封装过后十分精简易上手，出错概率低。同时支持移动客户端访问。系统会陆续更新一些实用功能。

性别男，Jack.Lin是给还没有出生女儿取的名字（寓意：你若不离不弃，我必生死相依）

Jack.Lin参考后台模板。有需要可自行到群内下载。

> 如需单应用，请移步 [Backstage-fast](https://gitee.com/y_project/Backstage-fast)  `(保持同步更新)`，如需其他版本，请移步 [项目扩展](http://jack.lin.vip/#/standard/xmkz)  `(不定时更新)`
...
演示地址：http://jack.lin.vip  

文档地址：http://jack.lin.vip
